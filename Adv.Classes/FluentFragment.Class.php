 ...
 * @method void Set_Elseif() adds keyword else into created code
 ...
 * @method void Set_Break() adds keyword break into created code
 * @method void Set_Continue() adds keyword continue into created code
 ...
 * @method void Set_Or2() adds || into created code
 ...
 * @method void Set_Smplqts() adds simple quotes/apostrophes '%s' into created code
 * @method void Set_Dblqts() adds double quotes "%s" into created code
 * @method void Set_Slshs() adds slashes /%s/ into created code
 * 
 ...
 * @method void Set_Or1() adds | into created code
 * 
...
class FluentFragment
{
	 ...
	 */
	private function __construct($InsertMode = Core::JASC_MODE_NOINS)
	{
		/*
	...
	public function __call($Method, $Parameters)
	{
		try
		{
			...
			{
				throw new JaSC_Exception(Core::UNICAT_XCPT_MAIN_CLS, Core::UNICAT_XCPT_MAIN_FNC, Core::UNICAT_XCPT_SEC_FNC_NOSUPPORT1);
			}
		}
		catch( JaSC_Exception $Exception )
		{
	...
	public static function Set_InsertMode($InsertMode)
	{
		try
		{
			if( empty($InsertMode) )
			{
				throw new UniCAT_Exception(Core::UNICAT_XCPT_MAIN_CLS, Core::UNICAT_XCPT_MAIN_FNC, Core::UNICAT_XCPT_MAIN_PRM, Core::UNICAT_XCPT_SEC_PRM_MISSING);
			}
			else
			{
	...
	private function Get_AvailableMethods()
	{
		 ...
		 * difference in name is only for purpose of "humanising", dividing of variable's name from variable's value in name of function;
		...
		foreach( ClassScope::Get_Interfaces('JaSC\I_JaSC_Options_Union') as $Interface )
		{
			...
			{
				$this -> Methods['Set_'.ucfirst(strtolower(explode('_', $Constant)[2]))] = array( 'Set_'.implode('', str_split(explode('_', $Interface)[3])), ClassScope::Get_ConstantValue($Interface, $Constant) );
			}
		}
	}
